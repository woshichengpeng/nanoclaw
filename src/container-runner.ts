...
function buildVolumeMounts(
  ...
  // Per-chatJid session directory (isolated from other groups AND platforms)
  // Each chat_jid gets their own session folder to prevent cross-platform session context leaking
  const sanitizedJid = (chatJid || group.folder).replace(/[^a-zA-Z0-9_-]/g, '_');
  const groupSessionsDir = path.join(DATA_DIR, 'sessions', group.folder, sanitizedJid, 'pi-sessions');
  ...
  const piAgentDirRaw = process.env.PI_CODING_AGENT_DIR || path.join(homeDir, '.pi', 'agent');
  const piAgentDir = piAgentDirRaw.startsWith('~/')
    ? path.join(homeDir, piAgentDirRaw.slice(2))
    : (piAgentDirRaw === '~' ? homeDir : piAgentDirRaw);
  ...
  // Only expose specific auth variables needed by Pi, not the entire .env
...
export async function runContainerAgent(
  ...
  const resolvedModel = input.model || getModelOverride() || DEFAULT_MODEL || undefined;
  const resolvedThinkingLevel = input.thinkingLevel || getThinkingLevelOverride() || undefined;
  ...
  const containerImage = CONTAINER_IMAGE;
  ...
  logger.info({
    ...
    thinkingLevel: resolvedThinkingLevel || '(default)',
  ...
  return new Promise((resolve) => {
    ...
    // For non-streaming mode: buffer stdout as before
    let stdout = '';
    ...
    container.stdout.on('data', (data) => {
      ...
      if (effectiveOnOutput) {
        ...
        // Non-streaming mode: buffer stdout
    ...
    container.on('close', (code) => {
      ...
      // Non-streaming mode: parse output from buffered stdout
      if (code !== 0) {
        logger.error({
          group: group.name,
          code,

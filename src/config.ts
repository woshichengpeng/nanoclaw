...
export const TASK_CONTAINER_TIMEOUT = parseInt(process.env.TASK_CONTAINER_TIMEOUT || '1800000', 10); // 30min for scheduled tasks
export const IDLE_TIMEOUT = parseInt(process.env.IDLE_TIMEOUT || '30000', 10); // 30s idle before closing container
...
const MODEL_OVERRIDE_PATH = path.join(DATA_DIR, 'model_override.json');
const THINKING_LEVEL_OVERRIDE_PATH = path.join(DATA_DIR, 'thinking_level_override.json');
...
export function getThinkingLevelOverride(): string | null {
  try {
    if (fs.existsSync(THINKING_LEVEL_OVERRIDE_PATH)) {
      const data = JSON.parse(fs.readFileSync(THINKING_LEVEL_OVERRIDE_PATH, 'utf-8'));
...
export function setThinkingLevelOverride(level: string | null): void {
  if (level) {
    fs.mkdirSync(path.dirname(THINKING_LEVEL_OVERRIDE_PATH), { recursive: true });
    fs.writeFileSync(THINKING_LEVEL_OVERRIDE_PATH, JSON.stringify({ level }, null, 2));
  } else {
    try { fs.unlinkSync(THINKING_LEVEL_OVERRIDE_PATH); } catch { /* ignore */ }
  }
}
